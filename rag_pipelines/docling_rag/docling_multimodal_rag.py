...
class DoclingMultiModalRAG:
    ...
    def __init__(self, config: AppConfig = {}) -> None:
        ...
        self.multimodal_reader = DoclingMultimodalReader(
    ...
    def add_documents(self, file_list: List[str]) -> None:
        ...
        for file in file_list:
            if file.lower().endswith('.pdf') or file.lower().endswith('.docx') or file.lower().endswith('.pptx'):
                # Use the multimodal reader to extract both text and images.
                docs.extend(
                    self.multimodal_reader.read_document(
                        file, 
    ...
    def get_context(self, query: str, k: int = 4) -> List[Document]:
        ...
        for doc in retrieved_docs:
            ...
            doc.metadata["tables"] = ast.literal_eval(doc.metadata["tables"])
    ...
    def get_answer(
        ...
        for doc in docs:
            ...
            doc_tables = doc.metadata.get("tables", [])
            doc_table_texts = [table["table_text"] for table in doc_tables]
            tables.extend(doc_table_texts)
